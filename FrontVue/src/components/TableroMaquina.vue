...
const nFicha = computed(() => juego.tablero.length);
let estadoPerspectiva = ref("noPerspectiva");
const usuario = useUsuarioStore()

const seleccion = ref(null);
const ganador = computed(()=>juego.ganador)

watch(ganador, (nuevoGanador) => {
  if (nuevoGanador === "jugador rojo") {
    datos.actualizarRanking(5);
    usuario.setMonedas(5);
  } else if (nuevoGanador === "jugador negro") {
    datos.actualizarRanking(-5);
    usuario.setMonedas(-5);
  }
});
...
function seleccionarCelda(i, j) {
  if (!seleccion.value) {
    ...
    if (esValido) {
      juego.moverPieza(seleccion.value, [i, j]);

      // Ejecutar IA tras movimiento humano (turno negras)
      setTimeout(() => {
        if (juego.turno === 'negras') {
          turnoIA();
        }
      }, 1000);
    }
...
// IA básica para turno de negras
function turnoIA() {
  const piezasIA = [];

  for (let i = 0; i < tablero.value.length; i++) {
    for (let j = 0; j < tablero.value[i].length; j++) {
      const pieza = tablero.value[i][j];
      if ([3, 4, 6, 13, 14, 16].includes(pieza)) {
        piezasIA.push([i, j]);
      }
    }
  }

  const movimientosDeAtaque = [];
  const movimientosSimples = [];

  for (const [x, y] of piezasIA) {
    const movimientos = juego.chequearMovimiento([x, y]);

    for (const [i, j] of movimientos) {
      const destino = tablero.value[i][j];
      const mov = { origen: [x, y], destino: [i, j] };

      if (destino !== 0) {
        movimientosDeAtaque.push(mov);
      } else {
        movimientosSimples.push(mov);
      }
    }
  }
  ...
  if (movimientosDeAtaque.length > 0) {
    seleccionIA = movimientosDeAtaque[Math.floor(Math.random() * movimientosDeAtaque.length)];
  } else if (movimientosSimples.length > 0) {
    seleccionIA = movimientosSimples[Math.floor(Math.random() * movimientosSimples.length)];
  } else {
    return; // No hay movimientos disponibles
  }

  juego.moverPieza(seleccionIA.origen, seleccionIA.destino);
}

</script>

<template>
  <v-btn @click="clasePerspectiva()" class="rounded-pill bontonPerspectiva">
    <v-icon>mdi-eye</v-icon>
  </v-btn>
  <v-btn @click="clasePerspectiva2()" class="rounded-pill bontonPerspectiva2">
    <v-icon>mdi-eye</v-icon>
  </v-btn>

  <div :class="['grid', estadoPerspectiva]" :style="{ gridTemplateColumns: `repeat(${nFicha}, 1fr)` }">
    <div v-for="(fila, i) in tablero" :key="i">
      <div
